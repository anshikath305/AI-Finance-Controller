...
def get_project_root():
    # Heuristic to find the root directory where 'data/' exists
...
async def upload_files(bank_file: UploadFile = File(...), ledger_file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    def save_txs(df, source, run_id):
        for _, row in df.iterrows():
            ...
            db.add(tx)
    save_txs(bank_df, "BANK", run.id)
    save_txs(ledger_df, "LEDGER", run.id)
    db.commit()
...
async def start_reconciliation(run_id: int, db: Session = Depends(get_db)):
    ...
    bank_df = pd.DataFrame([tx.__dict__ for tx in bank_txs])
    ledger_df = pd.DataFrame([tx.__dict__ for tx in ledger_txs])
    ...
    for res in results:
        bank_tx_id = bank_txs[res['bank_index']].id
        ledger_tx_id = ledger_txs[res['ledger_index']].id if res.get('ledger_index') is not None else None
        match = Match(run_id=run_id, bank_transaction_id=bank_tx_id, ledger_transaction_id=ledger_tx_id, status=res['status'], confidence=res['confidence'], matching_signals=res.get('signals', {}), explanation=res['explanation'])
        db.add(match)
        if res['status'] == 'UNRESOLVED' and ledger_tx_id is None:
             exc = ExceptionRecord(run_id=run_id, transaction_id=bank_tx_id, type="MISSING_LEDGER", description="No matching ledger entry found", severity="MEDIUM", status="OPEN")
             db.add(exc)
    run.status = "COMPLETED"
    run.processing_time = time.time() - start_time
...
async def get_matches(run_id: int, db: Session = Depends(get_db)):
    ...
    for m in matches:
        ...
        result.append({"id": m.id, "bank_transaction_id": m.bank_transaction_id, "ledger_transaction_id": m.ledger_transaction_id, "status": m.status, "confidence": m.confidence, "explanation": m.explanation, "matching_signals": m.matching_signals or {}, "bank_detail": bank_tx.raw_data if bank_tx else {}, "ledger_detail": ledger_tx.raw_data if ledger_tx else None})
    return result
...
async def review_match(match_id: int, action: ReviewAction, db: Session = Depends(get_db)):
    ...
    if action.action == "ACCEPT":
        ...
        match.ledger_transaction_id = None
...
async def list_benchmarks():
    return [
        {"id": "easy", "name": "Standard (Easy)", "difficulty": "EASY"},
        {"id": "medium", "name": "Medium (Mixed Scenarios)", "difficulty": "MEDIUM"},
        {"id": "hard", "name": "Hard (Adversarial)", "difficulty": "HARD"}
    ]
...
async def run_benchmark(benchmark_id: str):
    ...
    if not os.path.exists(bank):
        raise HTTPException(status_code=404, detail=f"Benchmark data not found at {bank}")
...
async def copilot_query(request: Dict[str, Any], db: Session = Depends(get_db), _ = Depends(rate_limiter.check_rate_limit)):
    ...
    if not run_id:
        raise HTTPException(status_code=400, detail="Missing runId in context")

    query = request.get("query", "")
    result = await copilot.answer_query(query, int(run_id), db)
    return result
